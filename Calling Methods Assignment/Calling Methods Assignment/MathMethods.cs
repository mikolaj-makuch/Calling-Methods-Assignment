...
namespace Calling_Methods_Assignment
{
    public class MathMethods
    {
        ...
        public static int Defense(int defense)
        {
            int enemy_str = 7;
            int weapon_mt = 11;
            int triangel_bonus = -1;
            int crit_coeff = 1;
            int damage = (enemy_str + (weapon_mt + triangel_bonus) - defense) * crit_coeff;
            return damage;
        }
        public static int Attack(int strength)
        {
            int enemy_def = 4;
            int weapon_mt = 8;
            int treangle_bonus = 1;
            int crit_coeff = 3;
            int damage = (strength + (weapon_mt + treangle_bonus) - enemy_def) * crit_coeff;
            return damage;
        }
    }
}
